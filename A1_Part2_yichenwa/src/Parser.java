...
class Stmts {   // stmts -> stmt [ stmts ]
	...
	public Stmts() {    // stmts -> stmt [stmts ]
		...
		s = new Stmt();
		if (Lexer.nextToken== Token.ID || Lexer.nextToken== Token.KEY_IF || Lexer.nextToken==Token.KEY_WHILE ||Lexer.nextToken==Token.LEFT_BRACE || Lexer.nextToken==Token.KEY_RETURN || Lexer.nextToken==Token.KEY_PRINT) {
			ss = new Stmts();
		}
...
class Cond extends Stmt {  // cond -> if '(' relexp ')' stmt [ else stmt ]
	...
	public Cond() {
		...
		r = new Relexp();
		Lexer.lex(); // skip over ')'
		int ifpoint = ByteCode.str_codeptr;//2
		ByteCode.skip(3);//change to 5
		c1 = new Stmt();
...
class Loop extends Stmt {   // loop -> while '(' relexp ')' stmt
	...
	public Loop() {
		...
		b = new Relexp();
		Lexer.lex();//skip )
		int anotherpoint = ByteCode.str_codeptr;
		//System.out.println("whilepoint="+whilepoint);
		ByteCode.skip(3);
		c = new Stmt();
		ByteCode.gen_goto(whilepoint);
		ByteCode.skip(2);
		ByteCode.patch(anotherpoint,ByteCode.str_codeptr);
	}
}
