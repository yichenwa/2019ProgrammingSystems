...
public class ByteCode {
	...
	public static void gen_goto(int n) {
	   ...
	   // missing code: to be filled in by you.#######################
		//System.out.println(str_code[str_codeptr]);
		String gt = "goto";
		code[codeptr]=gt;
		arg[codeptr]=n;
		codeptr++;
		if(n<0) {
			str_code[str_codeptr]=gt+" ";
		}
		else {
			str_code[str_codeptr]=gt+" "+n;
		}
		str_codeptr++;
	...
	public static void gen_invoke (int n)  {
		...
		// missing code: to be filled in by you.########################
		String inv = "invokevirtual";
		code[codeptr]=inv ;
		//code[codeptr+1]= String.valueOf(n) ;//add for part2
		arg[codeptr]=n;
		codeptr+=3;
		str_code[str_codeptr]=inv + " #" + n;
		str_codeptr+=3;
	}
